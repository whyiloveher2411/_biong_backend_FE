...
**Input:** `audio_script` + `caption-words.json` + `core_signals` + `creative_brief` + `totalVideoSec` + `visual_style` (CMS hoặc auto).
...
6. `assign-beat-prompt-types.mjs` bảo đảm `visual_description` → `map-shot-plan-to-beat-map.mjs` → `build-beat-element-timing.mjs`
7. Agent viết `compositions/beat_N.html` theo `visual_style` + `visual_description` + beat-timing → MCP media → render
...
|------------------|--------|
| `visual_style` | Phong cách chung toàn clip khi CMS = `auto` — xem [hyperframes-theme-init.md](hyperframes-theme-init.md) |
...
```json
{
  ...
  "visual_description": "Build a verified metric card with a restrained count reveal, supporting scale markers, and a final comparison frame that keeps the number dominant.",
...
> `visual_description` là brief tiếng Anh 8–80 từ. Nó mô tả content, composition và motion, nhưng không được tự đặt palette/font/theme.
...
| `visual_description` | Brief tiếng Anh 8–80 từ; bắt buộc khác beat liền kề; có thể nhắc exact media catalog ID |
...
3. `assign-beat-prompt-types.mjs` — bảo đảm `visual_description` hợp lệ
4. `map-shot-plan-to-beat-map.mjs` → `assets/beat-map.json`
5. `build-beat-element-timing.mjs` → `assets/beat-timing/beat_N.json`
6. **Mỗi beat:** đọc `@hyperframes/prompts/universal-composer.md` + `visual_style` + `visual_description` + timing JSON → viết `compositions/beat_N.html` (hf-seek `t`-based)
7. MCP media theo `accent_media` + `bg_media` + `/continuous-motion` ambient
8. Ghi `media-plan.md` với cột: `visual_description`, `accent_media`, `bg_media`
...
| Chỉ hiện chữ `phrase_anchor` trên nền | `visual_story` + `visual_description` có diễn tiến |
