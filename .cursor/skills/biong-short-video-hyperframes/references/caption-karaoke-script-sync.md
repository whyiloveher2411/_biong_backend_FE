...
1. **Đọc script** — `get_context.audio_script` → lưu `assets/audio-script.txt`; strip markers `[BGM: ...]`, `[Dừng Ns]`, `[SFX: ...]`, **và legacy non-verbal** (`[laughter]`/`[sigh]`/`[dissatisfaction-hnn]` nếu còn — xem omnivoice-expressive-tags.md; caption không hiển thị).
...
**Sau MCP TTS OmniVoice:** bắt buộc transcribe **lại** MP3 mới — prosody `. . .` làm đổi duration so với ước lượng script.
