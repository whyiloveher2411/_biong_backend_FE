...
| `hyperframes/prompts/{hf_prompt_type}.md` | `beats-draft/`, `compositions/beat_*.html` video khác |
...
- **Cấm** một template cố định cho 15 beat — mỗi beat **thiết kế riêng** theo prompt server + `hf_prompt_type`
...
2. Đọc `hyperframes/prompts/{hf_prompt_type}.md` (public hoặc server template)
