Ghép video agent cho short video ID 18 (import HTML chatbot — CHỈ ghép, KHÔNG sáng tạo visual mới).

Tiêu đề: Rủi ro lớn nhất khi dùng ChatGPT mỗi ngày không phải là những gì nó học được — mà là những điều bạn sẽ ngừng làm — Short video #1
...
Audio đã sẵn sàng: https://file-spacedev-app.spacedev.vn/uploads/short-video/agent-audio/18_vi_full.mp3?v=1783082223
render_mode=import_html | beats=15/15 | whisper_status=completed
hf_prompt_type gốc (tham khảo): universal-composer
data-duration mục tiêu: 173.25s

## Ghép lại từ HTML chatbot — nguồn truth = CMS (khớp preview admin) (render lúc 2026-07-05T11:07:21+07:00)
...
2. hyperframes init storage/agent-renders/18/my-video --example=vignelli
...
4b. **BẮT BUỘC** chuẩn hóa beat cho HyperFrames render (scaffolding — không sửa visual):
   node .cursor/skills/biong-short-video-preflight/scripts/normalize-import-html-beat-for-render.mjs storage/agent-renders/18/my-video --localize-images
...
8a. Bootstrap + BGM (bắt buộc trước wire index.html):
   node .cursor/skills/biong-short-video-preflight/scripts/bootstrap-phase2-assets.mjs storage/agent-renders/18/my-video
...
10. Preflight tối thiểu:
   node .cursor/skills/biong-short-video-preflight/scripts/check-import-html-beat-render.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/check-overlay-stack.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/verify-caption-sync.mjs storage/agent-renders/18/my-video --strict
   node .cursor/skills/biong-short-video-preflight/scripts/check-beat-timing.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/check-media-stack.mjs storage/agent-renders/18/my-video --strict
...
2. **Transcribe (bắt buộc):** node .cursor/skills/biong-short-video-preflight/scripts/transcribe-audio.mjs storage/agent-renders/18/my-video
...
3. **Pipeline sync (bắt buộc — cấm ghi tay Whisper vào caption):**
   ...
   node .cursor/skills/biong-short-video-preflight/scripts/sync-caption-from-script.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/verify-caption-sync.mjs storage/agent-renders/18/my-video --strict
   → FAIL: đọc assets/caption-sync-report.json → transcribe-audio lại → sync (tối đa 2 vòng) — KHÔNG render
   node .cursor/skills/biong-short-video-preflight/scripts/assign-beat-prompt-types.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/map-shot-plan-to-beat-map.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/build-beat-element-timing.mjs storage/agent-renders/18/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/sync-index-beats-from-map.mjs storage/agent-renders/18/my-video
   # Agent viết compositions/beat_N.html thủ công — CẤM gen-beats-from-shot-plan.mjs
   node .cursor/skills/biong-short-video-preflight/scripts/gen-captions-html.mjs storage/agent-renders/18/my-video
...
3. Nếu lỗi multipart/upload → CLI fallback:
   node mcp/short-video-agent/scripts/upload-agent-video.mjs --short-video-id 18 --file "<abs/output.mp4>"
...
- cp .cursor/skills/biong-short-video-hyperframes/assets/spacedev-logo.png → storage/agent-renders/18/my-video/assets/images/
- **Sinh watermark bằng script** (cấm hand-author sai #root):
  node .cursor/skills/biong-short-video-preflight/scripts/gen-brand-watermark.mjs storage/agent-renders/18/my-video --duration {totalVideoSec}
...
node .cursor/skills/biong-short-video-preflight/scripts/patch-stock-full-bleed.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-continuous-motion.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-stock-full-bleed.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-visual-density.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-typography-spacing.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-beat-timing.mjs storage/agent-renders/18/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-media-stack.mjs storage/agent-renders/18/my-video --strict
node .cursor/skills/biong-short-video-preflight/scripts/check-overlay-stack.mjs storage/agent-renders/18/my-video
...
## MCP tools
1. short_video_get_context({ short_video_id: 18 })
...
3. Bootstrap: node .cursor/skills/biong-short-video-preflight/scripts/bootstrap-phase2-assets.mjs storage/agent-renders/18/my-video
