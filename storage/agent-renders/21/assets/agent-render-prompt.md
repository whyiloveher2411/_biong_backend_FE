Render video agent cho short video ID 21 (phase 2 — video từ audio có sẵn).
...
1. node .cursor/skills/biong-short-video-preflight/scripts/capture-visual-audit.mjs storage/agent-renders/21/my-video
...
3. Lưu assets/agent-metadata.json (language + markers + timeline) từ get_context
   node .cursor/skills/biong-short-video-preflight/scripts/transcribe-audio.mjs storage/agent-renders/21/my-video
...
## HyperFrames CLI
- npx hyperframes init storage/agent-renders/21/my-video --non-interactive --skip-skills --example=vignelli --resolution portrait
