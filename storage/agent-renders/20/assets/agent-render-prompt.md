Render video agent cho short video ID 20 (phase 2 — video từ audio có sẵn).

Tiêu đề: Cha đẻ của Internet cho biết bài học lớn nhất từ mạng lưới này sắp trở thành 3 thách thức lớn nhất của AI — Short video #1
...
Audio đã sẵn sàng: https://file-spacedev-app.spacedev.vn/uploads/short-video/agent-audio/20_vi_full.mp3?v=1782990724
...
1. short_video_get_context → lưu **toàn bộ response** vào assets/get-context-snapshot.json
   node .cursor/skills/biong-short-video-preflight/scripts/bootstrap-phase2-assets.mjs storage/agent-renders/20/my-video
   (tạo assets/audio-script.txt + assets/agent-metadata.json từ audio_script + audio_script_metadata)
2. **Transcribe (bắt buộc):** node .cursor/skills/biong-short-video-preflight/scripts/transcribe-audio.mjs storage/agent-renders/20/my-video
...
3. **Pipeline sync (bắt buộc — cấm ghi tay Whisper vào caption):**
   ...
   node .cursor/skills/biong-short-video-preflight/scripts/sync-caption-from-script.mjs storage/agent-renders/20/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/verify-caption-sync.mjs storage/agent-renders/20/my-video --strict
   → FAIL: đọc assets/caption-sync-report.json → transcribe-audio lại → sync (tối đa 2 vòng) — KHÔNG render
   node .cursor/skills/biong-short-video-preflight/scripts/map-shot-plan-to-beat-map.mjs storage/agent-renders/20/my-video
   node .cursor/skills/biong-short-video-preflight/scripts/sync-index-beats-from-map.mjs storage/agent-renders/20/my-video
   # Agent viết compositions/beat_N.html thủ công — CẤM gen-beats-from-shot-plan.mjs
   node .cursor/skills/biong-short-video-preflight/scripts/gen-captions-html.mjs storage/agent-renders/20/my-video
...
3. Nếu lỗi multipart/upload → CLI fallback:
   node mcp/short-video-agent/scripts/upload-agent-video.mjs --short-video-id 20 --file "<abs/output.mp4>"
...
- cp .cursor/skills/biong-short-video-hyperframes/assets/spacedev-logo.png → storage/agent-renders/20/my-video/assets/images/
- **Sinh watermark bằng script** (cấm hand-author sai #root):
  node .cursor/skills/biong-short-video-preflight/scripts/gen-brand-watermark.mjs storage/agent-renders/20/my-video --duration {totalVideoSec}
...
node .cursor/skills/biong-short-video-preflight/scripts/patch-stock-full-bleed.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-continuous-motion.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-stock-full-bleed.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-visual-density.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-typography-spacing.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-beat-timing.mjs storage/agent-renders/20/my-video
node .cursor/skills/biong-short-video-preflight/scripts/check-media-stack.mjs storage/agent-renders/20/my-video --strict
node .cursor/skills/biong-short-video-preflight/scripts/check-overlay-stack.mjs storage/agent-renders/20/my-video
...
1. node .cursor/skills/biong-short-video-preflight/scripts/capture-visual-audit.mjs storage/agent-renders/20/my-video
...
1. short_video_get_context({ short_video_id: 20 })
...
3. Lưu assets/agent-metadata.json (language + markers + timeline) từ get_context
   node .cursor/skills/biong-short-video-preflight/scripts/transcribe-audio.mjs storage/agent-renders/20/my-video
...
Đọc @hyperframes-theme-init.md + @canvas-contract-3-layer.md + @layout-9x16-zones.md
1. **3 lớp** mỗi beat: dark gradient/grain nền → UI card glass → dynamic typography Be Vietnam Pro
2. Số liệu → npx hyperframes add stat-motion / apple-money-count — **cấm** plain div số
3. Support card title ≥36px; body ≥28px; gap giữa cards ≥24px; max 3 item/hàng ngang
4. **Layout:** `content-cluster` căn giữa dọc — chọn `layout_variant` (stack_center, vs_row, bento_2x2…); **cấm** screen split trái/phải trên 9:16
5. visual_enrichment[] từ creative_brief/core_signals — facts ngắn ngoài VO (không bịa số)
6. Beat ≥6s: ≥2 internal_acts hoặc ≥2 GSAP addLabel; meme/logo rotation:3 ease back.out(1.7)
7. Sau init theme `vignelli`: đọc CSS token project — không viết layout phẳng từ đầu

## HyperFrames CLI
- npx hyperframes init storage/agent-renders/20/my-video --non-interactive --skip-skills --example=vignelli --resolution portrait
