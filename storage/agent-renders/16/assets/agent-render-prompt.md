Render video agent cho short video ID 16 (phase 2 — video từ audio có sẵn).
...
## Motion Director — HF Prompt beats (bắt buộc)
Đọc @hf-prompt-art-direction.md + @hf-prompt-beat-contract.md + @hf-prompt-catalog.md + @layout-9x16-zones.md
1. Mỗi beat = 1 `hf_prompt_type` từ `hyperframes/prompts/` — đọc prompt file trước khi viết HTML
2. Element timing từ `assets/beat-timing/beat_N.json` (Whisper) — embed TIMINGS vào render()
3. Beat HTML: hf-seek `t`-based — **cấm** GSAP beat timeline, registry hero blocks
4. Nền transparent — ambient + stock bg ở index.html
5. Be Vietnam Pro local — @typography-be-vietnam-pro.md
6. visual_enrichment[] từ creative_brief — facts ngắn ngoài VO
7. Sau init theme `vignelli`: brand CSS vars từ prompt preamble (:root --cream/--ink/...)

## HyperFrames CLI
- npx hyperframes init storage/agent-renders/16/my-video --non-interactive --skip-skills --example=vignelli --resolution portrait
- **Cấm** --example=blank production
- Copy Be Vietnam Pro → assets/fonts/; strip Google Fonts CDN
- Đọc visual_shot_plan → ghi hf_theme nếu CMS=auto
