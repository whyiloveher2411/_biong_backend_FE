export function convertMSTime(time: number): string {
...
export function numberToThousoun(value: number | string): number | string {
    ...
    return Math.abs(Number(value)) >= 1.0e+9
        ...
        : Math.abs(Number(value)) >= 1.0e+6
            ...
            : Math.abs(Number(value)) >= 1.0e+3
                ? (Number((Math.abs(Number(value)) / 1.0e+3).toFixed(1))) + "K"
                : Math.abs(Number(value));
}

export function getValueType(value: ANY, type: string): string {
    switch (type) {
        case 'thumbnail':
...
export function findLinkMessage(str: string): string {
...
export function findCode(str: string): string {
    let result = str.match(/`([^`]+)`/g);
