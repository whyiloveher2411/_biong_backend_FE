...
export default function MarketingShortVideoPanel({ appMobileId }: Props) {
    const api = useAjax();
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {!loading && appMobileDetail?.post && (
                <>
                    <Typography variant="subtitle2">Danh sách short video theo mobile app</Typography>
                    <Box data-marketing-short-video-list="1">
                        <FieldForm
                            component="relationship_onetomany_show"
                            config={{
                                title: 'Short video',
                                object: 'spacedev_app_short_video',
                                field: 'app_mobile',
