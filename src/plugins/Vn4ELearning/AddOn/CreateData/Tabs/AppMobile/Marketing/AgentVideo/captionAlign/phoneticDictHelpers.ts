...
export type PhoneticDictEntry = {
    source_term: string;
    phonetic: string;
    phonetic_tokens?: string[];
};

export type PhoneticExpandSegment = {
    type: 'plain' | 'dict';
    sourceTerm: string;
    displayText: string;
    scriptStart: number;
    scriptCount: number;
    phoneticStart: number;
    phoneticCount: number;
    phoneticTokens: string[];
};

export type PhoneticExpandResult = {
    scriptWords: string[];
    phoneticWords: string[];
    segments: PhoneticExpandSegment[];
};
...
function scriptTokensMatch(slice, sourceTokens) {
    ...
    return slice.every((token, index) => normCompareToken(token) === normCompareToken(sourceTokens[index]));
}
...
function mergeAlignedEntries(entries) {
    ...
    const hasRed = matchTypes.some((type) => type === 'interpolate' || type === 'positional' || type === 'positional-gap');
    const hasYellow = matchTypes.some((type) => type.startsWith('cluster') || type === 'fuzzy' || type === 'phonetic-dict');
...
export function collapsePhoneticAlignToOriginal(
    ...
    const exactCount = mapped.filter((entry) => entry.matchType === 'exact' || entry.matchType === 'phonetic-dict-exact').length;
