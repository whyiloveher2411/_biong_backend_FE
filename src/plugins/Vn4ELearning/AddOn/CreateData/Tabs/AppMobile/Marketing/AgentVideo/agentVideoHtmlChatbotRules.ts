...
export function buildBeatDivisionSingleOutputRulesBlock(options?: {
    ...
    const durationRules = relax
        ...
        : beatFrequency !== 'free'
            ...
            : whiteboardDuration
            ? [
                '## Beat duration — timing (bắt buộc)',
                '- Segmentation theo **Layer A** (mỗi dòng không trống = 1 beat; transition phrase gộp với dòng kế).',
                '- `durationSec` = endSec - startSec theo Whisper thực tế — **không** ép vào range tần suất.',
                '- `phrase_anchor` = toàn bộ lời thoại thuộc beat (1 dòng hoặc transition phrase + dòng kế); `endSec` = Whisper end của từ cuối.',
                '- Pipeline **không** tách/gộp lại beat trong code — beat-map trả về giữ nguyên.',
                '- Chỉ bắt buộc: durationSec > 0, sections liên tục, không overlap/gap, **mỗi dòng không trống phải nằm trong đúng 1 beat** (transition phrase nhập vào beat kế).',
            ]
            : [
    ...
    return [
        ...
        "- Field string JSON (`visual_description`, `background`, `phrase_anchor`, …): **viết nội dung bình thường** — parser tự escape dấu nháy; `image_prompt` là **JSON object thật**.",
        '',
        '## Tự kiểm tra trước khi xuất (bắt buộc)',
        '- Trước khi trả response, kiểm tra tuần tự:',
        '  1. `sections` phủ liên tục từ 0 → `totalVideoSec` — không thiếu đầu/cuối.',
        '  2. **Không overlap** giữa 2 beat kề nhau.',
        '  3. **Không gap** giữa 2 beat kề nhau.',
        '  4. Mỗi beat đủ **7 key** trong `image_prompt` (subject, action, scene, text_overlay, composition, must_avoid, background_prompt); **bắt buộc** thêm `object_prompt_2..N` liên tục (số lớp = clamp(round(durationSec/2.5), 2, 6); chỉ beat <2s được 1 ảnh; 1 background dùng chung).',
        '  5. `image_prompt` **không có key thừa**: chỉ 7 key + (tùy chọn) `object_prompt_N` — cấm purpose/context/mood/style/`background_prompt_2`/….',
        ...
        '  12. **Subject Entity Leakage Check**: `subject` của mỗi beat được chọn **độc lập từ `phrase_anchor` của beat đó** — **cấm** kéo entity của beat trước làm subject cho beat sau (vd beat "Mutation" → subject phải là mutated DNA, không phải acetaldehyde molecule từ beat trước; beat "Cancer" → subject phải là cancer cells/tumor, không phải DNA).',
        '  13. **Đếm lớp ảnh**: mỗi beat ≥2s có `object_prompt_2..N` với số lớp = clamp(round(durationSec/2.5), 2, 6), N liên tục, các lớp kể tiếp nhau (không trùng nội dung) — beat 1 ảnh (thiếu `object_prompt_2`) là FAIL, phải bổ sung trước khi xuất.',
        '- Chỉ xuất beat-map sau khi cả 13 mục đều PASS.',
        '',
        ...durationRules,
        '',
    ].join('\n');
