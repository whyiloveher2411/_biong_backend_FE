...
function EditAccountNotifications(props: CreatePostAddOnProps) {
    ...
    React.useLayoutEffect(() => {
        ...
        ajax({
            url: `post-type/detail/vn4_account_notifications/${props.data.post.id}`,
            method: 'POST',
                ...
                unstable_batchedUpdates(() => {
                    if (result.redirect) {
                        ...
                        if (result.config) {
                            ...
                            result.type = 'vn4_account_notifications';
    ...
    const handleSubmit = () => {
        ...
        setData((prev: JsonFormat) => {
            ...
            if (!open) {
                ...
                ajax({
                    url: 'post-type/post/vn4_account_notifications',
                    method: 'POST',
                    data: {
                        ...prev.post,
                        _action: prev.action,
                    ...
                    success: (result: CreatePostTypeData) => {
                        ...
                        if (result.post) {
                            if (result.post.id !== prev.post.id) {
                                navigate(`/post-type/vn4_account_notifications/list?redirectTo=edit&post=${result.post.id}`);
                                return;
                            } else {
                                result.updatePost = new Date();
    ...
    if (data) {
        return (
            <Form
                ...
                postType={'vn4_account_notifications'}
...
export default EditAccountNotifications