...
const projectDir = path.join(repo, "storage/agent-renders/24/my-video");
const contextPath = path.join(repo, "storage/agent-renders/24/assets/get-context-snapshot.json");
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      title: ctx.title || "Short Video #24",
...
fs.writeFileSync(
  ...
  JSON.stringify({ render_mode: "import_html" }, null, 2),
);
